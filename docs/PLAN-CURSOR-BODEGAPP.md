...
- [~]  🔴 **Cerrar los XSS por `innerHTML`** escapando datos de BD en todas las vistas (nombres de productos/clientes en POS, cobranza, inventario, reportes, detalles de venta, indicadores). Relevante porque un XSS puede robar el JWT del panel/tokens de nube. *(PARCIAL: creado helper compartido `public/js/escape.js` (`escapeHtml`, con test unitario) y aplicado a los sinks de POS (nombres de producto/presentación) y Cobranza (nombre/cédula/teléfono/dirección de cliente). PENDIENTE aplicarlo a inventario, reportes, detalles_venta, indicadores, configuración, layout y etiquetas — requiere validación en GUI para no romper el render; ver Anexo B B.G.)*
...
- [x]  🔴 Definir dos ventanas configurables: `GRACE_OFFLINE_HOURS` (corta, p. ej. 72 h) y `HARD_OFFLINE_LIMIT` (a partir de la cual se **bloquea** aunque el token no haya expirado). *(`GRACE_OFFLINE_HOURS` (env, default 72) en `src/security/offline.js`; el `HARD_OFFLINE_LIMIT` efectivo es la expiración del token = `TOKEN_GRACE_DAYS`=5 días. El de 72h es el más estricto y el que bloquea primero.)*
- [x]  🔴 Si el servidor no responde: NO tratarlo como "válido". Permitir uso solo mientras `now - lastSuccessfulVerify < GRACE_OFFLINE_HOURS`; superado eso → **bloqueo total** con pantalla "reconéctate para validar tu licencia". *(`getCachedPayload` bloquea si se supera la ventana; `lastSeenEpoch` solo avanza tras un contacto exitoso con el servidor. La pantalla de activación ya se muestra al quedar EXPIRED — gate existente.)*
- [x]  🟠 Bajar la vida del token (`TOKEN_GRACE_DAYS` del servidor) para que la **revocación remota** se propague pronto (recomendado 3–7 días máx). *(Hecho en Fase 2 refuerzo: 5 días.)*
- [~]  🟠 El heartbeat debe reintentar con backoff y distinguir claramente `offline` (sin red) de `revoked/expired` (respuesta del servidor) — y en ambos casos aplicar la política de bloqueo correcta. *(La distinción offline vs revoked/expired YA existe en `heartbeat()` (limpia caché en revoked/expired; mantiene en offline hasta la ventana de gracia). El backoff explícito con reintentos se difiere — requiere validación en runtime/GUI.)*
...
- [x]  🔴 Eliminar el fallback portátil `device.id` en texto plano. Si `node-machine-id` falla, derivar el HWID de **múltiples señales del SO** (MachineGuid, serial de placa/BIOS por WMI, UUID de volumen del disco de sistema, modelo de CPU) y, si aun así no hay señales fuertes, **fallar de forma segura** (pedir activación online) en vez de generar un ID copiable. *(`src/security/hwid.js`: sin archivo portátil; `combineSignals` devuelve null si no hay señal fuerte → `getHardwareId()` null → el cliente no puede activar/validar (fail-safe).)*
- [x]  🟠 Combinar ≥3 señales y hashearlas (SHA-256) para el HWID final; documentar cuáles. *(Combina machineId, MachineGuid, board/BIOS serial, serial de volumen, CPU y platform-arch; SHA-256. Documentado en `src/security/hwid.js`.)*
- [x]  🟡 Ofuscar la `PUBLIC_KEY` embebida (no dejarla como bloque PEM literal evidente): cargarla troceada/derivada en runtime. No es secreto, pero dificulta el análisis y el parcheo automatizado. *(Guardada en base64 y decodificada en runtime; sin marcador `BEGIN PUBLIC KEY` literal en el fuente.)*
- [x]  🟠 En el servidor, añadir **detección de anomalías**: mismo HWID activándose/verificando desde muchas IPs o a alta frecuencia → alerta/limitar (mitiga clonado por VM y trial farming). *(Hecho en Fase 2 refuerzo: `trackAnomaly`.)*
...
- [x]  🟠 Incluir `jti` (id único) y `iat`/`exp` en el payload firmado del token (servidor). *(Fase 2 refuerzo: `jti` + `iat` + `exp` en `issueLicenseToken`.)*
- [x]  🟠 El cliente guarda el último `jti` visto y rechaza tokens con `iat` anterior al último aceptado (evita reutilizar un token viejo capturado). *(Módulo puro `src/security/token.js` (`isReplay`/`nextAcceptedIat`, 5 tests); `saveLicenseCache` persiste `lastAcceptedIat` monotónico y `getCachedPayload` rechaza tokens con `iat` menor.)*
- [~]  🟡 En LAN, servir la verificación siempre sobre canal controlado (ver Fase 3) para reducir sniffing/MITM. *(La verificación de licencia va contra el servidor local (`127.0.0.1`), no por LAN; HTTPS del servidor de licencias se abordará al migrar a VPS — ver Guía de migración.)*
...
- [~]  🔴 `verifyPassword()` retorna `true` cuando `adminPasswordHash` es `null`: sin contraseña configurada, cualquier verificación admin pasa. — `src/utils/auth.js:14-17` *(> DECISIÓN: es el comportamiento de primer arranque (aún no hay clave admin ⇒ no hay gate; así el dueño puede configurarla). `ensureUnlocked` también deja pasar cuando no hay clave. La protección real existe una vez el dueño fija la clave (bcrypt) — recomendado forzarlo en el onboarding. Cambiarlo a `false` bloquearía el primer uso.)*
...
- [x]  🔴 **Fallback HWID por archivo plano portátil.** Si `node-machine-id` falla (ej. MV, contenedor, o desinstalando el módulo), `getHardwareId()` cae a `getFallbackHardwareId()` que lee/escribe `device.id` en texto plano. Un atacante puede copiar este archivo + `lic.dat` de un equipo licenciado a otro, forzando que `node-machine-id` falle en el destino, y el HWID coincidirá. — `src/utils/license.js:31-41,49-51` *(Fase 11.4: eliminado el `device.id` portátil; HWID multi-señal con fail-safe.)*
- [~]  🔴 **Clave pública hardcodeada extraíble.** La `PUBLIC_KEY` RSA está en texto plano en `src/utils/license.js:16-24`. Cualquiera con acceso al binario (desempaquetando el ASAR de Electron) puede extraerla. Si bien no permite firmar tokens nuevos, permite inspeccionar la estructura de tokens y personalizar ataques. — *(Fase 11.4: ya no es un bloque PEM literal (base64 en runtime). Sigue siendo extraíble por definición (es pública); la ofuscación fuerte llega en Fase 13.)*
- [~]  🔴 **ASAR/Electron: el binario se puede desempaquetar y parchear.** Herramientas como `asar extract` permiten extraer todo el código JS. Un atacante puede modificar `verifyToken()`, `getAppStatus()`, `getHardwareId()`, o el `onRequest` hook de `server.js` para que siempre devuelvan estado válido. Luego re-empaqueta o reemplaza los archivos JS modificados y la app funciona sin licencia. — `src/utils/license.js:85-103,170-180`, `server.js:218-317` *(Mitigado en capas: self-check de integridad firmado (11.8, detecta parcheo en prod), primitivas de cifrado ligado a licencia (11.6, "quitar la licencia rompe la app"), y bytenode+ofuscación+fuses (Fase 13, en release). La "verdad de ingeniería" del plan asume que nada es 100% inextraíble; el objetivo es encarecer y detectar, cumplido en capas.)*
- [x]  🔴 **Expiración offline + manipulación del reloj (ataque combinado).** `verifyToken()` usa `Date.now()` del sistema (manipulable). Si un atacante: (1) activa trial, (2) atrasa el reloj al pasado, y (3) bloquea el servidor de licencias con firewall → el token nunca expira, el heartbeat falla como `offline` sin bloquear, y la app funciona indefinidamente. No hay verificación NTP ni bloqueo forzoso por offline prolongado. — `src/utils/license.js:96-99`, `controllers/license.controller.js:54-56` *(Fase 11.2: atrasar el reloj por debajo de `lastSeenEpoch-24h` invalida la caché. Fase 11.3: superada la ventana offline (72h) se bloquea aunque el token no haya expirado.)*
- [x]  🔴 **Heartbeat offline no bloquea.** En `heartbeat()`, si el servidor de licencias no responde (offline, firewall bloqueando `127.0.0.1:3000`, archivo `hosts` modificado), el error de red se trata como `reason: 'offline'` y NO se invalida la caché. Un atacante puede activar licencia/trial, luego bloquear todo el tráfico al servidor local con un firewall, y el token seguirá siendo válido hasta que expire su ventana de gracia (y si además manipula el reloj, nunca expira). — `controllers/license.controller.js:47-56` *(Fase 11.3: `getCachedPayload` bloquea al superar `GRACE_OFFLINE_HOURS` desde la última verificación exitosa; el token dura 5 días y la ventana offline dura 72h, así que el firewall ya no da uso indefinido.)*
- [x]  🟠 **Token sin nonce/jti ni replay protection.** `verifyToken()` solo valida: firma RSA, HWID, y expiración. No hay `jti`, `sequence number`, ni binding a sesión. Un token interceptado (sniffing localhost, ARP spoofing en LAN, o copia del disco) podría ser reutilizado en otra instancia si el atacante logra que el HWID coincida. — `src/utils/license.js:85-103` *(Fase 11.5: token con `jti`/`iat`; cliente rechaza `iat` menor al último aceptado (`lastAcceptedIat` en la caché).)*
...
- [~]  🟡 **Servidor de licencias sin HTTPS ni autenticación en endpoints públicos.** `license-server/server.js` usa HTTP plano y los endpoints `/activate`, `/verify`, `/trial` solo tienen rate limiting básico (20 req/min). Un atacante con ARP spoofing/MITM en la red local puede interceptar tokens y claves. — `license-server/server.js:210-295` *(> DECISIÓN: HTTPS se habilita al migrar a VPS (Guía de migración: Let's Encrypt, sin `rejectUnauthorized:false`). Hoy el servidor es LOCAL (`127.0.0.1`) por defecto; `/activate`/`/verify`/`/trial` son públicos por diseño (activación no autenticada) pero endurecidos: anti-`__proto__`, anti-replay (`jti`/`iat`), rate limiting y detección de anomalías. El token es inútil sin el HWID del equipo.)*
...
- [~]  🔴 Race conditions: patrón `readJson → modificar → saveJson` sin locks ni escritura atómica. — *(Mitigado: `saveJson` ahora es ATÓMICO (temp + rename) → un corte no corrompe el archivo; `readJson` respalda y no borra ante corrupción. El lock/serialización completa de la secuencia read-modify-write se deja para el VPS (hoy: un único admin, baja concurrencia).)*
...
- [~]  🟠 JWT guardado en `localStorage` del panel + XSS en `renderList`. — *(XSS CERRADO: `admin.html` ahora escapa `key`/`notas`/`equipo`/`hwid`/`plan`/`estado` con `esc()` (el vector entra por `systemName` no autenticado de `/activate`). El JWT sigue en `localStorage` (mover a cookie HttpOnly se hará al migrar el panel al VPS; sin XSS el robo del token vía script queda cerrado).)*
...
- [x]  🟠 **`network.json` en texto plano, editable por el usuario.** El archivo que controla `lanEnabled` está en `%APPDATA%/network.json` sin firma ni cifrado. Un atacante con acceso a los archivos puede activar LAN sin permiso. — `src/utils/network.js:11-34` *(Corregido: `network.json` se FIRMA con HMAC (secreto del cliente). Si la firma no valida (editado a mano), se ignora `lanEnabled` y se cae a `false` (fail-safe). Test añadido.)*
...
- [~]  🟠 `POST /api/print/remote` (printText/printHTML/getPrinters) sin token ni validación de origen. — `server.js` *(> DECISIÓN: se mantiene tras el gate de token temporal LAN (y licencia); es el endpoint que usa el móvil para imprimir. Añadir clave admin por impresión rompería el flujo móvil legítimo. En LAN el token temporal (15 min, del QR) es el control; documentado.)*
...
- [~]  🟠 Carpeta `uploads` servida estáticamente en `/uploads/` sin control de acceso; `parseMultipartUpload` usa la extensión del `filename` del cliente sin validar MIME/tamaño/whitelist. — `server.js:172-205,229-236` *(Mitigado: `parseMultipartUpload` ahora valida la extensión contra una whitelist por campo (imágenes para logo/imagen; xlsx/xls/csv para import) y rechaza el resto; `@fastify/multipart` limita el tamaño (20MB). Servir `/uploads/` tras el gate de licencia+LAN se mantiene; el control de acceso por archivo se evaluará si se guardan datos sensibles ahí.)*
...
- [~]  🔵 No hay Electron Fuses configurados (`RunAsNode`, `EnableNodeCliInspectArguments`, etc.). Positivo: `contextIsolation:true` y `nodeIntegration:false` ya están bien — no tocar. — `main.js` *(Fase 13 preparada: `scripts/apply-fuses.js` listo; se aplican al empaquetar en el entorno de release. Ver `docs/DISTRIBUCION.md` §6.)*
...
- [x]  🔴 `force_settle` cierra la venta como `PAGADO` y `monto_pendiente_usd = 0` sin verificar que la deuda recalculada sea realmente 0. — `controllers/client.controller.js:471-477` *(Corregido: solo cierra como PAGADO si la deuda recalculada es un residuo (<= $0.05); si queda deuda real, conserva el abono y el estado recalculado (no se pierde el pago) pero NO marca PAGADO.)*
...
- [x]  🟠 Búsqueda de ventas rota: patrón LIKE con espacios literales `` `% ${q}% ` `` casi nunca coincide. — `controllers/reports.controller.js:2898` *(Corregido: `%${q.trim()}%`.)*
...
- [x]  🟠 Consultas de abonos en reportes/cierre no filtran `COALESCE(a.anulado,0)=0` (a diferencia de `getAbonosBySaleIdStmt`), así abonos anulados podrían contar. — `controllers/reports.controller.js:62-69,142-158,221-236` *(Corregido: las 5 consultas que suman abonos ahora filtran `COALESCE(a.anulado,0)=0`. Además, `voidSale` ANULA los abonos (soft-delete) en vez de borrarlos. Importante tras el cambio de `voidPayment` a soft-delete.)*
- [~]  🟠 Al anular venta, `restoreStockStmt.run(...)` no verifica que el producto siga activo; si fue soft-deleted, el stock no se restaura. — `controllers/reports.controller.js:725-726` *(VERIFICADO contra el código: `getSaleProductsForSaleIdStmt` NO filtra por activo y `restoreStockStmt` hace `UPDATE ... WHERE id = ?` sin filtro de `activo`, así que el stock SÍ se restaura aunque el producto esté soft-deleted (la fila existe). Solo no restaura si `producto_id` es NULL (venta libre), que es correcto. El hallazgo no aplica al código actual.)*
...
- [x]  🟡 Cashea: `PagarCuota()` y `createCasheaVenta()` sin transacción ni validación de existencia/duplicados de la cuota/venta. — `controllers/cashea.controller.js:5-37,71-92` *(Corregido: `PagarCuota` valida existencia/estado y corre en transacción; `createCasheaVenta` valida cuotas (array, campos, que sumen el saldo financiado ±0.01) y rechaza duplicar el registro Cashea de una venta.)*
...
- [x]  🔵 **`getLicenseInfo` dispara heartbeat de red en cada GET `/api/license/info`** (`checkOnlineAndActivate().catch(()=>{})`), permitiendo verificaciones solapadas contra el servidor. — `controllers/license.controller.js:105` (Fase 14) *(Corregido: throttle de 30 s entre refrescos en background.)*
...
- [x]  🟡 **Cashea: cuotas no validan que sumen el total ni que las fechas sean válidas.** `createCasheaVenta` inserta las `cuotas` tal cual llegan del cliente sin verificar que `Σ monto_usd (+ inicial) == monto_total_usd` ni cantidad/orden de cuotas; permite planes de pago inconsistentes. Complementa A.4 (falta de validación en Cashea). — `controllers/cashea.controller.js:5-38` (Fase 5) *(Corregido junto a A.4: se valida que las cuotas sumen `monto_total_usd - monto_inicial_usd` (±0.01) y que cada cuota tenga `numero`/`monto_usd`/`fecha_vencimiento`.)*
