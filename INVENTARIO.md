...
| 2 | `license-server/server.js:12` | `SECRET_KEY` con fallback `'[REMOVED-COMPROMISED-SECRET]'` | **CRÍTICO** — firma JWT de admin | Env obligatorio, sin fallback (fail-fast). Fase 1 |
| 3 | `license-server/server.js:13` | `SHARED_API_KEY` con fallback `'[REMOVED-COMPROMISED-SECRET]'` | **CRÍTICO** — bypass de auth admin | Eliminar `authenticateApiKey`. Fase 1/2 |
...
| 5 | `src/utils/license.js:20` | `TRIAL_SECRET_KEY = '[REMOVED-COMPROMISED-SECRET]'` | MEDIO — HMAC del trial 72h | Mover a config/servidor; endurecer trial. Fase 1/2 |
| 6 | `src/utils/license.js:199` | `HIST_SECRET = '[REMOVED-COMPROMISED-SECRET]'` | MEDIO — cifra historial activación | Rotar/parametrizar. Fase 1/2 |
| 7 | `src/utils/auth.js:4` | `HASH_SECRET = '[REMOVED-COMPROMISED-SECRET]'` | **ALTO** — HMAC de clave admin local | Migrar a bcrypt/argon2. Fase 1/2/4 |
...
| 9 | `get-licenses.js` (todo el archivo) | Script que **incrusta** `SHARED_API_KEY`, `SECRET_KEY` y `admin=[REMOVED-COMPROMISED-CREDENTIAL]` para extraer licencias del server de producción | **CRÍTICO** — es en sí mismo una fuga | Eliminar del repo en Fase 1 |
