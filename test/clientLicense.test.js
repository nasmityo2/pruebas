...
const PRIVATE_KEY = fs.readFileSync(path.join(SERVER_DIR, 'private.key'), 'utf8');
...
test('cliente valida token de licencia correcto (firma + HWID + no expirado)', () => {
  ...
  const payload = licenseUtil.verifyToken(token);
...
test('cliente rechaza token con HWID de otro equipo', () => {
  ...
  assert.strictEqual(licenseUtil.verifyToken(token), null);
});
...
test('cliente rechaza token con firma manipulada', () => {
  ...
  assert.strictEqual(licenseUtil.verifyToken(tampered), null);
});
...
test('cliente rechaza payload alterado (firma no coincide)', () => {
  ...
  assert.strictEqual(licenseUtil.verifyToken(`${evil}.${sig}`), null);
});
